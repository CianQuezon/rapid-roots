...
class RootSolvers:
    ...
    def get_root(
        ...
        func_params = RootSolvers._standardise_func_params(
            func_params=func_params, n_problems=n_problems
        )
    ...
    def _standardise_func_params(
        func_params: Optional[npt.ArrayLike], n_problems: int
        ...
        if func_params.ndim == 0:
            ...
            if func_params.shape[0] != n_problems:
                raise ValueError(
                    f"func_params has {func_params.shape[0]} rows but expected {n_problems} rows"
                )
            return func_params
        else:
