...
class TestNewtonRaphsonVectorised:
    ...
    def test_no_parameters_multiple_solves(self):
        ...
        roots, iters, converged = _newton_raphson_vectorised(
            f, fp, func_params, x0, 1e-6, 50
        )
        
        # Compare with scipy
        expected = np.array([newton(lambda x: x**2 - 4, 1.5, fprime=lambda x: 2*x) 
                            for _ in range(100)])
    ...
    def test_single_parameter_multiple_solves(self):
        ...
        # Compare with scipy
        expected = np.array([newton(lambda x: x**2 - k, x0[i], fprime=lambda x: 2*x)
    ...
    def test_two_parameters_multiple_solves(self):
        ...
        # Compare with scipy
        expected = np.array([
            newton(lambda x: a_values[i] * x**2 - b_values[i], x0[i],
    ...
    def test_reshape_single_parameter_array(self):
        ...
        roots, iters, converged = _newton_raphson_vectorised(
            f, fp, k_values, x0, 1e-6, 50  # Pass 1D array directly
        )
...
class TestVectorisedConsistencyWithScipy:
    ...
    def test_consistency_various_sizes(self, n_solves):
        ...
        roots, _, converged = _newton_raphson_vectorised(
            f, fp, func_params, x0, 1e-8, 50
        )
    ...
    def test_davies_jones_wetbulb_scenario(self):
        ...
        roots, iters, converged = _newton_raphson_vectorised(
            simplified_wetbulb, simplified_wetbulb_prime,
            func_params, x0, 1e-6, 50
        )
