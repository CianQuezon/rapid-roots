...
class TestTryBackupOpenVectorised:
    ...
    def test_partial_convergence(self):
        ...
        assert converged[1] == True
        assert converged[3] == True
        assert converged[4] == True
    ...
    def test_comparison_comprehensive_scipy(self):
        ...
        test_scenarios = [
            ("Quadratic", quadratic_func, quadratic_scipy, quadratic_prime, 
             np.array([0.5, 1.0, 1.5, 2.5]), 2.0),
            ("Cubic", cubic_func, cubic_scipy, cubic_prime,
             np.array([0.5, 1.0, 1.5, 2.0]), 2**(1/3)),
            ("Transcendental", transcendental_func, transcendental_scipy, transcendental_prime,
             np.array([0.3, 0.5, 0.7, 1.0]), 0.739085),
        ]
        ...
        for name, func_jit, func_scipy, func_prime, x0_arr, expected_root in test_scenarios:
            # Your implementation
            roots = np.full(len(x0_arr), np.nan)
            iterations = np.full(len(x0_arr), 100)
            converged = np.full(len(x0_arr), False)
            ...
            if func_prime is not None:
                scipy_roots = np.array([
                    optimize.newton(func_scipy, x0, fprime=lambda x: func_prime(x), tol=1e-10)
                    for x0 in x0_arr
                ])
            else:
                scipy_roots = np.array([
                    optimize.newton(func_scipy, x0, tol=1e-10)
                    for x0 in x0_arr
                ])
...
class TestHelperFunctions:
    ...
    def test_update_converged_results_partial(self):
        ...
        _update_converged_results(
            ...
            unconverged_idx,
            updated_roots, updated_iterations, updated_converged
        )
        ...
        assert converged[0] == True  # Unchanged
        assert converged[1] == True  # Updated to True
        assert converged[2] == True  # Updated to True
        assert converged[3] == True  # Unchanged
        assert converged[4] == False  # Still False
