...
class TestCodegenOpenMethods:
    ...
    def test_no_parameters(self):
        ...
        # Compare with scipy
        expected = np.array([newton(lambda x: x**2 - 4, 1.5, fprime=lambda x: 2*x) for _ in range(100)])
    ...
    def test_single_parameter(self):
        ...
        # Compare with scipy
        expected = np.array([newton(lambda x: x**2 - k, 1.5, fprime=lambda x: 2*x) 
                            for k in k_values])
    ...
    def test_three_parameters_atmospheric(self):
        ...
        # Compare with scipy
        expected = np.array([
            newton(lambda T: T - params[i,1] - params[i,2] * (params[i,0] - T), x0[i],
...
class TestCodegenBracketMethods:
    ...
    def test_atmospheric_equation_four_params(self):
        ...
        expected = np.array([
            brentq(lambda T: T - params[i,1] - params[i,3] * 
                   (params[i,0] - T) * (params[i,2] / 101325.0), 
                   250.0, 300.0)
            for i in range(len(params))
        ])
