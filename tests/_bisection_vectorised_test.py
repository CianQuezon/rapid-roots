...
class TestBisectionVectorised:
    ...
    def test_two_parameters_multiple_solves(self):
        ...
        roots, iters, converged = _bisection_vectorised(
            f, func_params, a_bounds, b_bounds, 1e-6, 100
        )
        
        # Compare with scipy
        expected = np.array([
            brentq(lambda x: a_values[i] * x**2 - b_values[i], 
    ...
    def test_mixed_convergence(self):
        ...
        assert converged[0] == True   # offset = -4 (has real root)
        assert converged[1] == False  # offset = 1 (no real roots)
        assert converged[2] == True   # offset = -9 (has real root)
        assert converged[3] == False  # offset = 5 (no real roots)
        assert converged[4] == True   # offset = -16 (has real root)
    ...
    def test_reshape_single_parameter_array(self):
        ...
        roots, iters, converged = _bisection_vectorised(
            f, k_values, a, b, 1e-6, 100  # Pass 1D array directly
        )
...
class TestBisectionConsistencyWithScipy:
    ...
    def test_consistency_various_sizes(self, n_solves):
        ...
        # Compare with scipy
        expected = np.array([brentq(lambda x: x**2 - k, 0.0, b[i]) 
    ...
    def test_davies_jones_wetbulb_scenario(self):
        ...
        roots, iters, converged = _bisection_vectorised(
            simplified_wetbulb, func_params, a, b, 1e-6, 100
        )
