...
class TestTryBackupBracketVectorised:
    ...
    def test_multiple_functions(self):
        ...
        for func, a_arr, b_arr, expected_root in test_cases:
            ...
            assert success == True, f"Failed for function {func.__name__}"
    ...
    def test_invalid_brackets_returns_false(self):
        ...
        assert success == False, "Should return False for invalid brackets"
    ...
    def test_different_bracket_solvers(self):
        ...
        for solver, name in solvers:
            ...
            assert success == True, f"{name} should succeed"
    ...
    def test_comprehensive_scipy_comparison(self):
        ...
        test_scenarios = [
            ("Cubic", cubic_minus_8, cubic_minus_8_scipy,
             np.array([0.0, 0.5, 1.0, 1.5]), np.array([3.0, 3.0, 3.0, 3.0]), 2.0),
            ("Exponential", exponential_minus_e, exponential_minus_e_scipy,
             np.array([0.0, 0.3, 0.5, 0.8]), np.array([2.0, 2.0, 2.0, 1.5]), 1.0),
            ("Sine", sin_function, sin_function_scipy,
             np.array([3.0, 3.05, 3.1, 3.12]), np.array([3.5, 3.3, 3.2, 3.18]), np.pi),
            ("Logarithm", log_function, log_function_scipy,
             np.array([1.0, 1.5, 2.0, 2.5]), np.array([4.0, 4.0, 4.0, 4.0]), np.e),
        ]
        ...
        for name, func_jit, func_scipy, a_arr, b_arr, expected_root in test_scenarios:
            # Your implementation
            roots = np.full(len(a_arr), np.nan)
            iterations = np.full(len(a_arr), 100)
            converged = np.full(len(a_arr), False)
