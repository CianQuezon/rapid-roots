...
from shared.functions import get_function_by_name


def run_benchmark_functions_throughput(
    problem_size: list[int] = [10_000, 100_000, 1_000_000, 10_000_000],
    seed: int = 42,
    n_samples: int = 10,
    n_warmup: int = 3,
):
    ...
    for n_problem_samples in tqdm(problem_size, desc="Solving Problem Sizes: "):
        params, a_bounds, b_bounds = generate_test_samples(
            func_dict=func_dict, n_samples=n_problem_samples, seed=seed
        )
        x0_values = (a_bounds + b_bounds) / 2.0

        newton_results = _benchmark_rapid_roots_newton_throughput(
            func_dict=func_dict,
            params=params,
            x0_values=x0_values,
            n_problem_samples=n_problem_samples,
            n_samples=n_samples,
            n_warmup=n_warmup,
        )
        brent_results = _benchmark_rapid_roots_brent_throughput(
            func_dict=func_dict,
            params=params,
            a_bounds=a_bounds,
            b_bounds=b_bounds,
        ...
        bisect_results = _benchmark_rapid_roots_bisect_throughput(
            func_dict=func_dict,
            params=params,
            a_bounds=a_bounds,
            b_bounds=b_bounds,
...
def _benchmark_rapid_roots_newton_throughput(
    func_dict: Dict,
    params: npt.NDArray,
    x0_values: npt.NDArray,
    ...
    for i in range(n_samples):
        ...
        _ = _benchmark_rapid_roots_newton(
            func_dict=func_dict, params=params, x0_values=x0_values
        )
...
def _benchmark_rapid_roots_brent_throughput(
    func_dict: Dict,
    params: npt.NDArray,
    a_bounds: npt.NDArray,
    b_bounds: npt.ArrayLike,
    n_problem_samples: int,
    n_samples: int = 10,
    n_warmup: int = 3,
):
    ...
    for i in range(n_samples):
        start = time.perf_counter()
        _ = _benchmark_rapid_roots_brent(
            func_dict=func_dict, params=params, a_bounds=a_bounds, b_bounds=b_bounds
        )
        elapsed = time.perf_counter() - start
        times.append(elapsed)
...
def _benchmark_rapid_roots_bisect_throughput(
    func_dict: Dict,
    params: npt.NDArray,
    a_bounds: npt.NDArray,
    b_bounds: npt.NDArray,
    n_problem_samples: int,
    n_samples: int = 10,
    n_warmup: int = 3,
):
    ...
    for i in range(n_samples):
        start = time.perf_counter()
        _ = _benchmark_rapid_roots_bisect(
            func_dict=func_dict, params=params, a_bounds=a_bounds, b_bounds=b_bounds
        )
        elapsed = time.perf_counter() - start
        times.append(elapsed)
...
if __name__ == "__main__":
    run_benchmark_functions_throughput(
        problem_size=[10_000, 100_000, 1_000_000, 10_000_000],
        seed=42,
        n_samples=13,
        n_warmup=3,
    )
