...
def run_benchmark_single_function(
    ...
    rapid_roots_brent_results = _benchmark_rapid_roots_brent(
        func_dict, params, a_bounds, b_bounds
    )

    brent_error_metrics = calculate_error_metrics(
        scipy_brent_results, rapid_roots_brent_results
    )
    results["brent"] = {
    ...
    # Bisection Method
    scipy_bisect_results = _benchmark_scipy_bisect(
        func_dict, params, a_bounds, b_bounds
    )
    rapid_roots_bisect_results = _benchmark_rapid_roots_bisect(
        func_dict, params, a_bounds, b_bounds
    )

    bisect_error_metrics = calculate_error_metrics(
        scipy_bisect_results, rapid_roots_bisect_results
    )
    results["bisect"] = {
    ...
    rapid_roots_newton_results = _benchmark_rapid_roots_newton(
        func_dict, params, x0_values
    )

    newton_error_metrics = calculate_error_metrics(
        scipy_newton_results, rapid_roots_newton_results
    )
    results["newton"] = {
